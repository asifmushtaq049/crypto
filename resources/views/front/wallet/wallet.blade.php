  ...
  <div class="container items company">
    ...
    @foreach($wallets as $key => $wallet)
      <div class="row">
          ...
          <div class="col-sm-2">
            Coins<br/>
            <span class="value">{{$wallet->coins}}</span>
          </div>
          <div class="col-sm-3">
            <input type="hidden" class="rating-tooltip-manual blue" data-filled="fa fa-star fa-2x rating-color" value="{{ number_format($wallet->rating ? $wallet->rating->avg('stars') : 0, 1, '.', ',') }}" data-empty="fa fa-star-o fa-2x" data-fractions="2" readonly/>
          </div>
          <div class="col-sm-2">
            <a href="/wallet/{{$wallet->id}}" class="btn btn-info pull-right">READ MORE >></a>
          </div>
      </div>
      @endforeach
