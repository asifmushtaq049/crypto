...
<section>
  ...
  <div class="container item">
    <div class="section-header">
      <h2>{{$data['company']->name}}
        <form action="" method="post" class="pull-right">
          @csrf
          @if(Auth::check())
            @if($data['company']->wishlist->where('company_id',$data['company']->id)->where('user_id', Auth::user()->id)->first())
      ...
      <div class="row">
        ...
        <div class="col-lg-8 content clear">
          ...
          <form class="review-form" method="post" id="comment-form">
            @csrf
            @if($data['company']->rating->where('company_id',$data['company']->id)->where('user_id', Auth::user()->id)->first())
            <h2>Leave a Review <input name="stars" value="{{$data['company']->rating->where('company_id',$data['company']->id)->where('user_id', Auth::user()->id)->first()->stars}}" disabled type="hidden" class="rating-tooltip-manual blue pull-right" data-filled="fa fa-star fa-2x rating-color" data-empty="fa fa-star-o fa-2x" data-fractions="2"/></h2>
            @else
